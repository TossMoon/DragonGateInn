...
function (_transaction) {
  _inherits(branchAddRoomTransaction, _transaction);
  ...
  _createClass(branchAddRoomTransaction, [{
    ...
    value: function execute() {
      ...
      (_get2 = _get(_getPrototypeOf(branchAddRoomTransaction.prototype), "execute", this)).call.apply(_get2, [this].concat(args)); //检查参数是否符合要求
      ...
      args.forEach(function (item, index) {
        if (index > 0) {
          SingletonFactory.getInstance(allRoomManager).getOneRoomManagerByBranchId(args[0]).addRoom(item);
        }
      });
    }
  }]);
